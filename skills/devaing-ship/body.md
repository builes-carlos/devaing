...
## Step 0b — Verify phase is shippable

Read `CONTEXT.md ## Phases`. Find the current phase row (the one with the highest Phase number).

Check its Status:

- **Status = `In Progress`**: stop and refuse. Count open issues for the phase:
  ```bash
  gh issue list --state open --json number,milestone | jq '[.[] | select(.milestone != null)] | length'
  ```
  Output:
  ```
  ⚠ Phase "<name>" still In Progress (<N> issues open).
  Finish or revise the phase before shipping:
    /devaing-work             Implement remaining issues
    /devaing-phase-revise     Adjust scope if some issues should be cut
  ```
...
If tag found: store as `<last-ship-tag>`. Then verify there's something new to ship:

```bash
git log <last-ship-tag>..HEAD --oneline | wc -l
```

If 0 commits since tag: stop with `Nothing to ship. Phase "<name>" is already shipped (<last-ship-tag>) and no new commits.`
...
If this was a phase-complete deploy:

1. Update `CONTEXT.md ## Phases` — change the shipped phase's Status to `Complete`.

2. Archive the phase to `CONTEXT_ARCHIVE.md`:
   - If `CONTEXT_ARCHIVE.md` does not exist, create it with this header:
     ```markdown
     # CONTEXT_ARCHIVE

     Phases shipped to production. Moved from CONTEXT.md by devaing-ship.
     To reconstruct full history, read this file alongside CONTEXT.md.
     ```
   - Append to `CONTEXT_ARCHIVE.md`:
     ```markdown
     ## Phase <N> — <phase-name> (shipped <YYYY-MM-DD> as <tag-name>)

     <the exact row from the Phases table for this phase>
     ```
   - In `CONTEXT.md ## Phases`, replace that phase's table row with:
     `<!-- Phase <N> archived in CONTEXT_ARCHIVE.md -->`
   - Do NOT move `## Architecture`, `## Domain glossary`, `## Key constraints`, or `## Known limitations` — they accumulate across phases and always stay in `CONTEXT.md`.

3. Commit + push:
   ```bash
   git add CONTEXT.md CONTEXT_ARCHIVE.md
   git commit -m "docs: archive Phase <N> to CONTEXT_ARCHIVE.md after ship"
   git push
   ```
